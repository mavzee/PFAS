...
- **Tracker (FedEx)** (dashboard): enter a **Tracking ID** while logged in. The app calls the Firebase callable **`trackFedEx`** (Puppeteer on the server) to fetch travel history automatically. If FedEx blocks the scrape or it times out (~1 minute), use the **paste manually** fallback: open [FedEx.com](https://www.fedex.com/fedextrack/), click **View more details**, copy the travel history `tbody` HTML from DevTools, paste, and extract. Deploy functions: `npm run deploy:functions` from the project root (requires `FEDEX_ENABLE_WEB_SCRAPE=true` on the function; set to `false` to disable server scrape).
